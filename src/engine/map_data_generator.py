...
class MapDataGenerator:
    def __init__(self, size, grid_size, R):
        self.size = size
        self.grid_size = grid_size
        self.R = R

    def generate_map_data(self, vec_data):
        x_size, y_size = self.size
        assert x_size % self.grid_size == 0 and y_size % self.grid_size == 0
        x_bins = np.arange(0, x_size + 1, self.grid_size)
        y_bins = np.arange(0, y_size + 1, self.grid_size)
        ...
        for i in range(len(y_bins) - 1):
            for j in range(len(x_bins) - 1):
                center_pos = np.array(
                    [
                        self.grid_size * (j + 1 / 2),
                        self.grid_size * (i + 1 / 2),
                    ]
                )
                ...
                crs, local_density = self.calc_crs(around_data, center_pos, self.R)
                crs_map[i, j] = crs
                density_map[i, j] = local_density

        return crs_map, density_map

    def calc_crs(self, around_data, center_pos, R):
