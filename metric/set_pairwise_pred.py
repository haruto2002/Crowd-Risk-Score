...
def process_target_info(args):
    ...
    for scene_id in ["1", "2"]:
        frame_range = target_info[scene_id]["frame_range"]
        crop_area = target_info[scene_id]["crop_points"]
        ...
        crs_list.append(float(crs_mean))
        density_list.append(float(density_mean))

    return i, {"crs": crs_list, "density": density_list}


def set_pairwise_pred(path2dataset, pred_dir):
